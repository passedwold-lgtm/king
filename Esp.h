...
// Function Pointers for New Features
...
float colorEsp[3] = {255, 255, 255}; // White
static int Box = 0;
float AimFov = 0.0f;
...
Vector3 GetHeadPosition(void* player) {
    ...
    void *HeadTF = get_transformFF(HeadITF); //ITransformNode OLCJOGDHJJJ
...
monoString* get_Nickname(void* player) {
    monoString* (*nickname)(void* instance) = (monoString * (*)(void*)) getRealOffset(ENCRYPTOFFSET("0x1056EA918")); //   public string get_NickName() { }   VA *****1/18*****
...
Vector3 GetHipPosition(void* player) {
    void *HipITF= *(void **)((uintptr_t) player + 0x4D8);
    void *HipTF = get_transformFF(HipITF);  //ITransformNode OLJBCONDGLO
    Vector3 Hip = getPosition(HipTF);
    return Hip;
}

Vector3 GetToePosition(void* player) {
    void *ToeITF= *(void **)((uintptr_t) player + 0x510); //ITransformNode AGHJLIMNPJA
...
static bool get_isVisible(void *player) {
    bool (*_get_isVisible)(void *players) = (bool (*)(void *))getRealOffset(ENCRYPTOFFSET("0x1056F2510"));
    return _get_isVisible(player);
}

static void *Component_GetTransform(void *player) {
    void *(*_Component_GetTransform)(void *component) = (void *(*)(void *))getRealOffset(ENCRYPTOFFSET("0x106366CE8"));  //   public Transform get_transform() { }
    return _Component_GetTransform(player);
}

static void SwapWeapon(void* player, int POFFNNMOOBM, bool GDKLMFLNNGM) {
    void (*_SwapWeapon)(void* player, int POFFNNMOOBM, bool GDKLMFLNNGM) = (void (*)(void *, int, bool))getRealOffset(ENCRYPTOFFSET("0x105875044"));
...
static bool Physics_Raycast(Vector3 camLocation, Vector3 headLocation, unsigned int LayerID, void* collider) {
    bool (*_Physics_Raycast)(Vector3 camLocation, Vector3 headLocation, unsigned int LayerID, void* collider) = (bool(*)(Vector3, Vector3, unsigned int, void*))getRealOffset(ENCRYPTOFFSET("0x104E783FC"));//public static bool PLDCHDBCOBF(Vector3 OOFIJHADLNI, Vector3 CHGADBAMMOP, uint ONEDHFJBCMJ, ref MADMMIICBNN JEEIBOEGGPD) { }
    return _Physics_Raycast(camLocation, headLocation, LayerID, collider);
}

static void *Player_GetHeadCollider(void *player) {
    void *(*_Player_GetHeadCollider)(void *players) = (void *(*)(void *))getRealOffset(ENCRYPTOFFSET("0x1056ED460"));//public static bool PLDCHDBCOBF(Vector3 OOFIJHADLNI, Vector3 CHGADBAMMOP, uint ONEDHFJBCMJ, ref MADMMIICBNN JEEIBOEGGPD) { }
...
static bool get_isLocalTeam(void *player) {
    bool (*_get_isLocalTeam)(void *players) = (bool (*)(void *))getRealOffset(ENCRYPTOFFSET("0x105706324")); 
    return _get_isLocalTeam(player);
}

static bool get_IsDieing(void *player) {
    bool (*_get_die)(void *players) = (bool (*)(void *))getRealOffset(ENCRYPTOFFSET("0x1056E3F34")); 
    return _get_die(player);
}

static int GetHp(void* player) {
    int (*_GetHp)(void* players) = (int(*)(void *))getRealOffset(ENCRYPTOFFSET("0x105744710")); 
...
static int get_MaxHP(void* enemy) {
    int (*_get_axHP)(void* player) = (int(*)(void *))getRealOffset(ENCRYPTOFFSET("0x1057447B8")); 
...
static void *IsLocalObServer() {
    void *(*_GetocalPlayerOrObServer)(void *players) = (void *(*)(void *))getRealOffset(ENCRYPTOFFSET("0x1025F1DBC")); 
    return _GetocalPlayerOrObServer(NULL);
}

static void *get_MyFollowCamera(void *players) {
    void *(*_get_yFollowCamera) (void *player) = (void *(*)(void *))getRealOffset(ENCRYPTOFFSET("0x1056ED040")); 
    return _get_yFollowCamera(players);
}

static Vector3 GetAttackableCenterWS(void *player) {
    Vector3 (*_GtAttackableCenterWS)(void *players) = (Vector3 (*)(void *))getRealOffset(ENCRYPTOFFSET("0x105803B88")); 
    return _GtAttackableCenterWS(player);
}

Vector3 CameraMain(void* player){
    return getPosition(*(void**) ((uint64_t) player + 0x248));
}

static bool get_IsSighting(void *player) {
    bool (*_get_Isighting)(void *players) = (bool (*)(void *))getRealOffset(ENCRYPTOFFSET("0x1056E45AC")); 
    return _get_Isighting(player);
}
static bool get_IsCrouching(void *player) {
    bool (*_get_IsCrouching)(void *players) = (bool (*)(void *))getRealOffset(ENCRYPTOFFSET("0x105708A1C")); 
    return _get_IsCrouching(player);
}
static bool get_IsFiring(void *player) {
    bool (*_get_IsFiring)(void *players) = (bool (*)(void *))getRealOffset(ENCRYPTOFFSET("0x1056E44F8")); 
    return _get_IsFiring(player);
}

static Vector3 GetForward(void *player) {
    Vector3 (*_GetForward)(void *players) = (Vector3 (*)(void *))getRealOffset(ENCRYPTOFFSET("0x1063AC3CC")); 
    return _GetForward(player);
}

static void set_aim(void *player, Quaternion look) {
    void (*_set_aim_nepmods)(void *players, Quaternion lock) = (void (*)(void *, Quaternion))getRealOffset(ENCRYPTOFFSET("0x1056EEAFC")); 
    _set_aim_nepmods(player, look);
}

// New Feature Implementations via Direct Offsets
static void NinjaRun_Update(void* player, bool enable) {
    if (!player) return;
    void (*_func)(void*, bool) = (void (*)(void*, bool))getRealOffset(ENCRYPTOFFSET("0x105708A1C")); // Example offset for NinjaRun
    _func(player, enable);
}

static void FlyJump_Update(void* player, bool enable) {
    if (!player) return;
    void (*_func)(void*, bool) = (void (*)(void*, bool))getRealOffset(ENCRYPTOFFSET("0x1056E45AC")); // Example offset for FlyJump
    _func(player, enable);
}

static void AutoFly_Update(void* player, bool enable) {
    if (!player) return;
    void (*_func)(void*, bool) = (void (*)(void*, bool))getRealOffset(ENCRYPTOFFSET("0x1056E44F8")); // Example offset for AutoFly
    _func(player, enable);
}

static void Move_Update(void* player, float x, float y, float z) {
    if (!player) return;
    void (*_func)(void*, float, float, float) = (void (*)(void*, float, float, float))getRealOffset(ENCRYPTOFFSET("0x1063ABAF0")); // Example offset for Move
    _func(player, x, y, z);
}

static void Stop_Update(void* player) {
    if (!player) return;
    void (*_func)(void*) = (void (*)(void*))getRealOffset(ENCRYPTOFFSET("0x105875044")); // Example offset for Stop
    _func(player);
}
            ...
            if (players != nullptr && camera != nullptr) {
                for (int u = 1; u < players->getNumValues(); u++) {
                    ...
                    if (ClosestEnemy != local_player && ClosestEnemy != nullptr && get_isVisible(ClosestEnemy) && !get_isLocalTeam(ClosestEnemy) && get_MaxHP(ClosestEnemy)) {
                        ...
                        if (ESPLine) {
                            // Fix the line to connect to the top of the box
                            ImVec2 lineEndPoint = top_pos;
                            
                            // If no name is displayed, adjust the line endpoint slightly
                            if (!ESPName) {
                                lineEndPoint.y -= 5; // Small offset to make it look cleaner
                            }

                            RenderLine(ImVec2(kWidth / 2, 0), lineEndPoint, ImColor(colorEsp[0], colorEsp[1], colorEsp[2]), 0.9);

                            if (ESPBox) {
                                float height = abs(enemyPos.y - top_pos.y);
                                float width = height * 0.65f;
                                DrawBox(top_pos.x - (width * 0.5), top_pos.y, width, height, ImColor(255, 255, 255), 0.9, 0.9);
                            }
                            ...
                            if (ESPName) {
                                std::string nickname = IsClientBot(ClosestEnemy) ? "BOT" : getPlayerNickname(ClosestEnemy);
                                // Fix the pointer dereference for nicknameWidth
